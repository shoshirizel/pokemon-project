...
def chain(pokemon_name):
    ...
    evolution_chain_url = requests.get(
        url=species_url, verify=False).json()['evolution_chain']['url']
    return requests.get(
        url=evolution_chain_url, verify=False).json()['chain']
...
def evolve_to(chain, name):
    ...
    new_poke = requests.get(
        f"https://pokeapi.co/api/v2/pokemon/{new_name}", verify=False).json()
    return new_poke
